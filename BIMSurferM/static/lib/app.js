...
function BimSurfer() {
	...
	this.loadGeometry = function(roid, serializerOid) {
		...
		$(".loadingdiv").append("<div class=\"progress\"><div class=\"bar\" style=\"width: 0%\"></div></div>");
		$(".loadingdiv").show();
		othis.log("downloading  " + roid + ";classname " + className + ";serializerOid " + serializerOid); 
	...
	this.loadScene = function(scene) {
		...
		try {
			...
			if (othis.scene != null) {
				...
				var ref, len, i, unit, sizingFactor,properties;
				...
				//locate the latitude and gratitude;yunxie
				properties = othis.scene.data().properties;
				othis.scene.refLatitude = 0;
				othis.scene.refLongitude = 0;
				$.each(properties,function (key){
					if(properties[key].hasOwnProperty("Ref Latitude")){
						othis.scene.refLatitude = properties[key]["Ref Latitude"];
					}
					if(properties[key].hasOwnProperty("Ref Longtitude")){
						othis.scene.refLongitude = properties[key]["Ref Longtitude"];
					}
				});
				othis.log("refLatitude:" + othis.scene.refLatitude + ":Longitude" + othis.scene.refLongitude );
				return othis.scene;
			}
		} catch (error) {
			othis.log(error);
